...
try
{
  ...
  dtack_drawing_mode_comreq_c.prototype = new dtack_drawing_mode_base_c();
										// override the constructor
  dtack_drawing_mode_comreq_c.prototype.constructor = dtack_drawing_mode_comreq_c;
										/* remember who the base class is */
  //dtack_drawing_mode_comreq_c.prototype.parent = dtack_drawing_mode_base_c.prototype;
}
catch(exception)
{
...
function dtack_drawing_mode_comreq_c(page)
{
										// we are not doing a prototype construction?
  if (arguments.length > 0)
  {
	var F = "dtack_drawing_mode_comreq_c";
...
// -------------------------------------------------------------------------------
dtack_drawing_mode_comreq_c.prototype.initialize = function(definition_object)
{
  var F = "initialize";
...
dtack_drawing_mode_comreq_c.prototype.activate = function(currently_active_mode_object)
{
  var F = "activate";
...
dtack_drawing_mode_comreq_c.prototype.onmousedown = function(event_object)
{
  var F = "onmousedown";

  if (this.page.canvas.inert)
    return;
  ...
  this.debug(F, "creating vertex at " +
    ...
    vertex.x + "," + vertex.y +
	" at factor " + factor +
    " with this.definition_object2.dtack_auto_edge " + this.definition_object2.dtack_auto_edge);
  ...
  vertex.definition_object = new Object();
										...
										/* let mode object impart some properties */
  vertex.disable_halo = this.definition_object2.disable_halo;
  vertex.show_label = this.definition_object2.show_label;
  ...
  if (this.definition_object2.dblclick_for_footnotes == "yes")
  {
	...
	var footnote_label = this.page.peek_next_footnote_label();
										...
										// eztask #6725: default note type to be IR
	var footnote_type = this.definition_object2.default_footnote_type;
	if (footnote_type == undefined)
	  footnote_type = "";
...
dtack_drawing_mode_comreq_c.prototype.draw = function(uuid, edge_uuid, x, y, label)
{
  var F = "draw";
  ...
  var vertex = this.create_vertex(uuid, edge_uuid, x, y, label)
...
dtack_drawing_mode_comreq_c.prototype.create_vertex = function(uuid, edge_uuid, x, y, label)
{
  var F = "create_vertex";
  ...
  var anchor_x = undefined;
  if (this.definition_object2.anchor_x != undefined)
    anchor_x = parseInt(this.definition_object2.anchor_x);
  if (anchor_x == undefined || isNaN(anchor_x))
    anchor_x = Math.round(xe/2.0);

  var anchor_y = undefined;
  if (this.definition_object2.anchor_y != undefined)
    anchor_y = parseInt(this.definition_object2.anchor_y);
  if (anchor_y == undefined || isNaN(anchor_y))
    anchor_y = Math.round(ye/2.0);
                                                 
  //alert(F + ":\n" + "text at " + (x - anchor_x) + "," + (y - anchor_y));
  ...
  var symbol = this.page.canvas.raphael.image(
	...
	xe, ye);
                                        ...
                                        // allow a label to be emplaced on the symbol
                                        // for now, expecting a short 1-2 character label  
  if (label)
    this.page.canvas.raphael.text(
      x - 2, 
      y - 1,
      label);

										/* wrap a vertex object around the primitive */
...
dtack_drawing_mode_comreq_c.prototype.vertex_dblclicked = function(event_object, vertex)
{
  ...
  if (this.definition_object2.dblclick_for_footnotes == "yes")
  {
	...
	this.page.edit_comreq(x, y, vertex);
  }
  else
  {
...
// -------------------------------------------------------------------------------
// handle mouseout on a vertex

dtack_drawing_mode_comreq_c.prototype.vertex_mouseout = function(event_object, vertex)
{
  var F = "vertex_mouseout";
