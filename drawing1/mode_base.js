...
dtack_drawing_mode_base_c.prototype.ajax_created_vertex = function(ajax)
{
  ...
  else
  {
										...
										// watchfrog #38
	this.page.reject_user_action(F, "could not create new " + name + ": " + ajax.message);
  }
...
dtack_drawing_mode_base_c.prototype.ajax_moved_vertex = function(ajax)
{
  ...
  {
										/* let the page display acknowledgement of this */
	this.page.acknowledge_user_action(F, name + " moved");
  }
  else
  {
										/* let the page display error message */
										// watchfrog #38
	this.page.reject_user_action(F, "unable to move " + name + ": " + ajax.message);
  }
...
dtack_drawing_mode_base_c.prototype.vertex_moved = function(vertex)
{
  ...
  this.ajax_move_vertex(
	vertex.uuid,
	(vertex.unzoomed_x != undefined? vertex.unzoomed_x: vertex.x) + vertex.anchor_x,
	(vertex.unzoomed_y != undefined? vertex.unzoomed_y: vertex.y) + vertex.anchor_y);
...
{
                                        // use the dtack_utility to calculate mouse-object relative position in mode_base
                                        // watchfrog #135
  return dtack_utility_relative_coordinates(event, reference);
} // end method
